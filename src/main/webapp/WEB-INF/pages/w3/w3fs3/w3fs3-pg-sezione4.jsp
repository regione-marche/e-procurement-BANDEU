...
<gene:formScheda entita="W3FS3" gestisciProtezioni="true"
	gestore="it.eldasoft.sil.w3.tags.gestori.submit.GestoreW3FS3" >
	...
	<gene:campoScheda>
		<td colspan="2"><b><br>${gene:resource("label.simap.fs3.IV")}<br><br></b></td>
	</gene:campoScheda>
	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.1")}</b></td>
	</gene:campoScheda>
	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.1.1")}</b></td>
	</gene:campoScheda>	
	<gene:campoScheda campo="TYPE_PROCEDURE" obbligatorio="true"/>
	<gene:campoScheda campo="ACCELERATED" />
	
	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.1.3")}</b></td>
	</gene:campoScheda>		
	<gene:campoScheda campo="FRAMEWORK" />
	<gene:campoScheda campo="DPS" />

	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.1.6")}</b></td>
	</gene:campoScheda>		
	<gene:campoScheda campo="IS_ELECTRONIC" />	
	
	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.1.8")}</b></td>
	</gene:campoScheda>		
	<gene:campoScheda title="L'appalto &egrave; disciplinato dall'accordo sugli appalti pubblici ?" campo="CONTRACT_COVERED_GPA" />		
	
	<gene:campoScheda>
		<td colspan="2"><b><br>${gene:resource("label.simap.fs3.IV.2")}</b></td>
	</gene:campoScheda>
	<gene:campoScheda>
		<td colspan="2"><b>${gene:resource("label.simap.fs3.IV.2.1")}</b></td>
	</gene:campoScheda>	
	<gene:campoScheda title="Numero dell'avviso nella GU/S" campo="NOTICE_NUMBER_OJ" />
	
	<gene:campoScheda>
		<td colspan="2"><b><br>${gene:resource("label.simap.fs3.IV.2.8")}</b></td>
	</gene:campoScheda>	
	...
	<gene:campoScheda>
		<td colspan="2"><b><br>${gene:resource("label.simap.fs3.IV.2.9")}</b></td>
	</gene:campoScheda>	
	...
	<gene:fnJavaScriptScheda funzione="gestioneTYPE_PROCEDURE('#W3FS3_TYPE_PROCEDURE#')" elencocampi="W3FS3_TYPE_PROCEDURE" esegui="true" />
	
</gene:formScheda>

<gene:javaScript>

	$("#W3FS3_TYPE_PROCEDURE option").each(function() {
		$(this).text($(this).attr("title"));
	});

	function gestioneTYPE_PROCEDURE(type_procedure) {
		if (type_procedure == '9' || type_procedure == '5' || type_procedure == '7') {
			$("#rowW3FS3_ACCELERATED").show();					
		} else {
			$("#rowW3FS3_ACCELERATED").hide();
			$("#W3FS3_ACCELERATED").val("");
		}
	}
