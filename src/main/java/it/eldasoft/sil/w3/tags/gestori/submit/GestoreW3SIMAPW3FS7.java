...
public class GestoreW3SIMAPW3FS7 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS7.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS7-preInsert: inizio metodo ");
    ...
    // Salvataggio W3FS7 e figlie
    datiForm.setValue("W3FS7.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS7());

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS7-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS7-preUpdate: inizio metodo ");

    // Salvataggio W3FS1 e figlie
    if (datiForm.getColumn("W3FS7.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS7.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS7());
    } else {
      this.update(status, datiForm, new GestoreW3FS7());
    }

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS1-preUpdate: fine metodo ");
