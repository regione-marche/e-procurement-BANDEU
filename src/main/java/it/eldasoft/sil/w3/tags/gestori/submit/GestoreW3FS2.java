...
public class GestoreW3FS2 extends AbstractGestoreEntita {
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    datiForm.setValue("W3FS2S36.ID", datiForm.getColumn("W3FS2.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS2S36());

    // Gestione ed inserimento del primo lotto obbligatorio in W3ANNEXB
    datiForm.setValue("W3ANNEXB.ID", datiForm.getColumn("W3FS2.ID").getValue().getValue());
  ...
  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS2-preUpdate: inizio metodo ");

    if (datiForm.getColumn("W3FS2S36.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS2S36.ID", datiForm.getColumn("W3FS2.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS2S36());
    } else {
      this.update(status, datiForm, new GestoreW3FS2S36());
    }
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS2-preUpdate: fine metodo ");
  }
  ...
  private void gestioneW3CRITERIA(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    this.gestisciAggiornamentiRecordSchedaMultipla(status, datiForm, gestoremultiploW3CRITERIA, "W3CRITERIA",
        new DataColumn[] { datiForm.getColumn("W3FS2.ID") }, null);
  }
