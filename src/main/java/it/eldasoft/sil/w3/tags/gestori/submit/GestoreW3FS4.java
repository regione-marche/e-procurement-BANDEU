...
public class GestoreW3FS4 extends AbstractGestoreEntita {
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    // Gestione ed inserimento della riga nella tabella di estensione W3FS2S2
    AbstractGestoreEntita gestoreW3FS4S2 = new DefaultGestoreEntita("W3FS4S2", this.getRequest());
    datiForm.setValue("W3FS4S2.ID", datiForm.getColumn("W3FS4.ID").getValue().getValue());
    this.inserisci(status, datiForm, gestoreW3FS4S2);

    // Gestione ed inserimento del primo lotto obbligatorio in W3ANNEXB
    datiForm.setValue("W3ANNEXB.ID", datiForm.getColumn("W3FS4.ID").getValue().getValue());
  ...
  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    AbstractGestoreEntita gestoreW3FS4S2 = new DefaultGestoreEntita("W3FS4S2", this.getRequest());
    if (datiForm.getColumn("W3FS4S2.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS4S2.ID", datiForm.getColumn("W3FS4.ID").getValue().getValue());
      this.inserisci(status, datiForm, gestoreW3FS4S2);
    } else {
      this.update(status, datiForm, gestoreW3FS4S2);
    }
  ...
  private void gestioneW3LANGUAGE(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    this.gestisciAggiornamentiRecordSchedaMultipla(status, datiForm, gestoremultiploW3LANGUAGE, "W3LANGUAGE",
        new DataColumn[] { datiForm.getColumn("W3FS4.ID") }, null);
  }
