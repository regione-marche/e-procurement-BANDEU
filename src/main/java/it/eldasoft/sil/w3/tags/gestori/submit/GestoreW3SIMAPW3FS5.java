...
public class GestoreW3SIMAPW3FS5 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS5.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS5-preInsert: inizio metodo ");
    ...
    // Salvataggio su W3FS5 e figlie
    datiForm.setValue("W3FS5.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS5());

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS5-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS5-preUpdate: inizio metodo ");

    // Salvataggio su W3FS5 e figlie
    if (datiForm.getColumn("W3FS5.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS5.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS5());
    } else {
      this.update(status, datiForm, new GestoreW3FS5());
    }

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS5-preUpdate: fine metodo ");
