...
public class GestoreCampoSECTION_FS5 extends AbstractGestoreCampo {
  ...
  protected void initGestore() {
    ...
    Vector chiaviFS5 = new Vector();

    while (resourceEnumeration.hasMoreElements()) {
      String chiave = (String) resourceEnumeration.nextElement();
      if (chiave.startsWith("label.simap.fs5.")) chiaviFS5.add(chiave);
    }

    Enumeration enumerationFS5 = chiaviFS5.elements();
    List listFS5 = Collections.list(enumerationFS5);
    Collections.sort(listFS5);

    this.getCampo().addValore("", "");

    for (int i = 0; i < listFS5.size(); i++) {
      String chiave = (String) listFS5.get(i);
      String valore = UtilityTags.getResource(chiave, null, false);

      if (!"label.simap.fs5.II.1.3".equals(chiave) && !"label.simap.fs5.IV.1.1".equals(chiave)) {
        if (valore.length() > 100) valore = valore.substring(0, 100);
        this.getCampo().addValore(chiave.replace("label.simap.fs5.", ""), valore);
      }
    }
