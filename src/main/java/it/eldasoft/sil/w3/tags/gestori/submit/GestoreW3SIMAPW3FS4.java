...
public class GestoreW3SIMAPW3FS4 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS4.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS4-preInsert: inizio metodo ");
    ...
    // Salvataggio W3FS4 e figlie
    datiForm.setValue("W3FS4.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS4());
    
    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS4-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS4-preUpdate: inizio metodo ");

    // Salvataggio W3FS4 e figlie
    if (datiForm.getColumn("W3FS4.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS4.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS4());
    } else {
      this.update(status, datiForm, new GestoreW3FS4());
    }

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS4-preUpdate: fine metodo ");
