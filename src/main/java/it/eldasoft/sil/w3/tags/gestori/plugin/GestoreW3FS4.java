...
public class GestoreW3FS4 extends AbstractGestorePreload {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3FS4.class);

  public GestoreW3FS4(BodyTagSupportGene tag) {
    super(tag);
  }
  ...
  public void doBeforeBodyProcessing(PageContext page, String modoAperturaScheda) throws JspException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS4: inizio metodo");
    ...
    if (!(UtilityTags.SCHEDA_MODO_INSERIMENTO).equals(modoAperturaScheda)) {
      ...
      Long id = new Long(((JdbcParametro) keyParent.get("W3FS4.ID")).getStringValue());
      ...
      try {
        ...
        String reviewBodyNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs4 "
            + " where uffint.codein = w3fs4.REVIEW_BODY_CODEIN "
            + " and w3fs4.id = ?", new Object[] { id });

        String mediationBodyNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs4 "
            + " where uffint.codein = w3fs4.MEDIATION_BODY_CODEIN "
            + " and w3fs4.id = ?", new Object[] { id });
        String reviewInfoNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs4 "
            + " where uffint.codein = w3fs4.REVIEW_INFO_CODEIN "
            + " and w3fs4.id = ?", new Object[] { id });
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS4: fine metodo");
