...
public class GestoreW3SIMAPW3FS1 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS1.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS1-preInsert: inizio metodo ");
    ...
    datiForm.setValue("W3FS1.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS1());
    
    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS1-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS1-preUpdate: inizio metodo ");

    // Salvataggio W3FS1 e figlie
    if (datiForm.getColumn("W3FS1.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS1.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS1());
    } else {
      this.update(status, datiForm, new GestoreW3FS1());
    }
