...
public class GestoreW3SIMAPW3FS6 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS6.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS6-preInsert: inizio metodo ");
    ...
    // Salvataggio su W3FS6 e figlie
    datiForm.setValue("W3FS6.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS6());
    
    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS6-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS6-preUpdate: inizio metodo ");

    if (datiForm.getColumn("W3FS6.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS6.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS6());
    } else {
      this.update(status, datiForm, new GestoreW3FS6());
    }
    
    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS6-preUpdate: fine metodo ");
