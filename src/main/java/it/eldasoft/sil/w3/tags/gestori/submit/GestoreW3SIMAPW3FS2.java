...
public class GestoreW3SIMAPW3FS2 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS2.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS2-preInsert: inizio metodo ");
    ...
    // Salvataggio su W3FS2 e figlie
    datiForm.setValue("W3FS2.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS2());

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS2-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS2-preUpdate: inizio metodo ");

    // Salvataggio su W3FS2 e figlie
    if (datiForm.getColumn("W3FS2.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS2.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS2());
    } else {
      this.update(status, datiForm, new GestoreW3FS2());
    }

    this.gestioneW3SIMAP_ADDR(status, datiForm);

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS2-preUpdate: fine metodo ");
