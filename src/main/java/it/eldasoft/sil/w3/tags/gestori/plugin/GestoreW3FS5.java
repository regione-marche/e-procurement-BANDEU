...
public class GestoreW3FS5 extends AbstractGestorePreload {
   ...
   * @param tag
   */
  public GestoreW3FS5(BodyTagSupportGene tag) {
    super(tag);
  }
  ...
  public void doBeforeBodyProcessing(PageContext page, String modoAperturaScheda) throws JspException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS5: inizio metodo");
    ...
    if (!(UtilityTags.SCHEDA_MODO_INSERIMENTO).equals(modoAperturaScheda)) {
      ...
      Long id = new Long(((JdbcParametro) keyParent.get("W3FS5.ID")).getStringValue());
      ...
      try {
        ...
        String appealProcedureNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs5s36 "
            + " where uffint.codein = w3fs5s36.appeal_procedure_codein "
            + " and w3fs5s36.id = ?", new Object[] { id });

        String mediationProcedureNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs5s36 "
            + " where uffint.codein = w3fs5s36.mediation_procedure_codein "
            + " and w3fs5s36.id = ?", new Object[] { id });

        String appealServiceNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs5s36 "
            + " where uffint.codein = w3fs5s36.appeal_service_codein "
            + " and w3fs5s36.id = ?", new Object[] { id });
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS5: fine metodo");
