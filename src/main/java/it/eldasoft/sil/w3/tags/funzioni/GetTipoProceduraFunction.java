...
public class GetTipoProceduraFunction extends AbstractFunzioneTag {

  public GetTipoProceduraFunction() {
    ...
    if (chiave != null && chiave.length() > 0) {
      ...
      try {
        typeProcedure = (String) sqlManager.getObject(
            "select type_procedure from w3fs3 where id = ?",
            new Object[] { id });
      } catch (SQLException s) {
        throw new JspException(
            "Errore durante la lettura del tipo di procedura", s);
      }
    }
