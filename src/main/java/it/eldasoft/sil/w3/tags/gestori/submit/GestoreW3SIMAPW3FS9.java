...
public class GestoreW3SIMAPW3FS9 extends AbstractGestoreChiaveIDAutoincrementante {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3SIMAPW3FS9.class);
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS9-preInsert: inizio metodo ");

    super.preInsert(status, datiForm);
    datiForm.setValue("W3FS9.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
    this.inserisci(status, datiForm, new GestoreW3FS9());
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS9-preInsert: fine metodo ");

  }

  public void preUpdate(TransactionStatus status, DataColumnContainer datiForm) throws GestoreException {

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS9-preUpdate: inizio metodo ");

    if (datiForm.getColumn("W3FS9.ID").getValue().getValue() == null) {
      datiForm.setValue("W3FS9.ID", datiForm.getColumn("W3SIMAP.ID").getValue().getValue());
      this.inserisci(status, datiForm, new GestoreW3FS9());
    } else {
      this.update(status, datiForm, new GestoreW3FS9());
    }

    if (logger.isDebugEnabled()) logger.debug("GestoreW3SIMAPW3FS9-preUpdate: fine metodo ");
