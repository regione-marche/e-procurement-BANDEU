...
public class GestoreW3FS7 extends AbstractGestorePreload {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3FS7.class);

  public GestoreW3FS7(BodyTagSupportGene tag) {
    super(tag);
  }
  ...
  public void doBeforeBodyProcessing(PageContext page, String modoAperturaScheda) throws JspException {
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS7: inizio metodo");
    ...
    if (!(UtilityTags.SCHEDA_MODO_INSERIMENTO).equals(modoAperturaScheda)) {
      ...
      Long id = new Long(((JdbcParametro) keyParent.get("W3FS7.ID")).getStringValue());
      ...
      try {
        ...
        String reviewBodyNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs7 "
            + " where uffint.codein = w3fs7.REVIEW_BODY_CODEIN "
            + " and w3fs7.id = ?", new Object[] { id });

        String mediationBodyNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs7 "
            + " where uffint.codein = w3fs7.MEDIATION_BODY_CODEIN "
            + " and w3fs7.id = ?", new Object[] { id });
        String reviewInfoNomein = (String) sqlManager.getObject("select uffint.nomein from uffint, w3fs7 "
            + " where uffint.codein = w3fs7.REVIEW_INFO_CODEIN "
            + " and w3fs7.id = ?", new Object[] { id });
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS7: fine metodo");
