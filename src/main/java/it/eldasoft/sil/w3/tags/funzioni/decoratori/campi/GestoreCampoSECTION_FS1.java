...
public class GestoreCampoSECTION_FS1 extends AbstractGestoreCampo {
  ...
  protected void initGestore() {
    ...
    Vector chiaviFS1 = new Vector();

    while (resourceEnumeration.hasMoreElements()) {
      String chiave = (String) resourceEnumeration.nextElement();
      if (chiave.startsWith("label.simap.fs1.")) chiaviFS1.add(chiave);
    }

    Enumeration enumerationFS1 = chiaviFS1.elements();
    List listFS1 = Collections.list(enumerationFS1);
    Collections.sort(listFS1);

    this.getCampo().addValore("", "");

    for (int i = 0; i < listFS1.size(); i++) {
      String chiave = (String) listFS1.get(i);
      String valore = UtilityTags.getResource(chiave, null, false);

      if (!"label.simap.fs1.II.1.3".equals(chiave) && !"label.simap.fs1.IV.1.1".equals(chiave)) {
        if (valore.length() > 100) valore = valore.substring(0, 100);
        this.getCampo().addValore(chiave.replace("label.simap.fs1.", ""), valore);
      }
    }
