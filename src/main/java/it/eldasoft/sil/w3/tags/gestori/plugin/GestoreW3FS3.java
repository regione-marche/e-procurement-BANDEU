...
public class GestoreW3FS3 extends AbstractGestorePreload {

  /** Logger */
  static Logger logger = Logger.getLogger(GestoreW3FS3.class);

  /**
   * 
   * @param tag
   */
  public GestoreW3FS3(BodyTagSupportGene tag) {
    super(tag);
  }
  ...
  public void doBeforeBodyProcessing(PageContext page, String modoAperturaScheda)
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS3: inizio metodo");
...

    {
      ...
      Long id = new Long(
          ((JdbcParametro) keyParent.get("W3FS3.ID")).getStringValue());

      try {
        String appealProcedureNomein = (String) sqlManager.getObject(
            "select uffint.nomein from uffint, w3fs3 "
                + " where uffint.codein = w3fs3.appeal_procedure_codein "
                + " and w3fs3.id = ?", new Object[] { id });

        String mediationProcedureNomein = (String) sqlManager.getObject(
            "select uffint.nomein from uffint, w3fs3 "
                + " where uffint.codein = w3fs3.mediation_procedure_codein "
                + " and w3fs3.id = ?", new Object[] { id });

        String appealServiceNomein = (String) sqlManager.getObject(
            "select uffint.nomein from uffint, w3fs3 "
                + " where uffint.codein = w3fs3.appeal_service_codein "
                + " and w3fs3.id = ?", new Object[] { id });
    ...
    if (logger.isDebugEnabled()) logger.debug("GestoreW3FS3: fine metodo");
