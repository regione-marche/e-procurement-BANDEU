...
public class ValidazioneFS4 extends ValidazioneFormulariStandard {

  static Logger logger = Logger.getLogger(ValidazioneFS4.class);

  /**
   * Controllo dati formulario standard 4: avviso di preinformazione.
   * 
   * @param sqlManager
   * @param id
   ...
   */
  public void validazioneFS4(SqlManager sqlManager, Long id, List<Object> listaControlli) throws GestoreException {
    if (logger.isDebugEnabled()) logger.debug("validazioneFS4: inizio metodo");
    try {

      // Tipo di preinformazione
      Long notice_f04 = (Long) sqlManager.getObject("select notice_f04 from w3fs4 where id = ?", new Object[] { id });
      if (notice_f04 == null) {
        ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "NOTICE_F04", "");
      }
      this.validazione_FS4_Sezione_I(sqlManager, id, listaControlli);
      this.validazioneFS4_Sezione_II(sqlManager, id, listaControlli);
      this.validazioneNumeroMassimoLotti(sqlManager, id, listaControlli);
      this.validazione_FS4_W3ANNEXB(sqlManager, id, null, listaControlli);
      this.validazioneFS4_Sezione_III(sqlManager, id, listaControlli);
      this.validazioneFS4_Sezione_IV(sqlManager, id, listaControlli);
      this.validazioneFS4_Sezione_VI(sqlManager, id, listaControlli);

    } catch (SQLException e) {
      throw new GestoreException("Errore nella lettura delle informazioni relative all'avviso di preinformazione", "validazioneFS4", e);
    }
    if (logger.isDebugEnabled()) logger.debug("validazioneFS4: fine metodo");
  }

  /**
   * Validazione della sezione I: amministrazione aggiudicatrice. Versione
   * specializzata per FS4.
   * 
   * @param sqlManager
   * @param id
   * @param listaControlli
   ...
   * @throws GestoreException
   */
  public void validazione_FS4_Sezione_I(SqlManager sqlManager, Long id, List<Object> listaControlli) throws SQLException, GestoreException {
    ...
    try {
      ...
      if (datiW3SIMAP != null && datiW3SIMAP.size() > 0) {
        ...
        if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
   ...
   * @throws GestoreException
   */
  private void validazioneFS4_Sezione_II(SqlManager sqlManager, Long id, List<Object> listaControlli) throws SQLException, GestoreException {

    String selectFS4 = "select w3fs4.notice_f04, " // 0
        + " w3fs4s2.title_contract, " // 1
        + " w3fs4s2.cpv, " // 2
        + " w3fs4s2.type_contract, " // 3
        + " w3fs4s2.scope_total, " // 4
        + " w3fs4s2.div_into_lots, " // 5
        + " w3fs4s2.div_lots, " // 6
        + " w3fs4s2.div_lots_max, " // 7
        + " w3fs4s2.lots_max_tenderer, " // 8
        + " w3fs4s2.date_publi, " // 9
        + " w3fs4s2.scope_cost, " // 10
        + " w3fs4s2.reference, " // 11
        + " w3fs4s2.lots_combining " // 12
        + " from w3fs4, w3fs4s2 "
        + " where w3fs4.id = w3fs4s2.id and w3fs4.id = ? and w3fs4s2.num = 1";

    List<?> datiFS4 = sqlManager.getVector(selectFS4, new Object[] { id });

    String pagina = "";

    Long notice_f04 = (Long) SqlManager.getValueFromVectorParam(datiFS4, 0).getValue();

    // Denominazione dell'appalto
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.1", null, false);
    String title_contract = (String) SqlManager.getValueFromVectorParam(datiFS4, 1).getValue();
    if (title_contract == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "TITLE_CONTRACT", pagina);
    } else {
      ValidazioneUtility.validazioneS200(title_contract, "W3FS4S2", "TITLE_CONTRACT", pagina, listaControlli);
    }

    // Numero di riferimento
    String reference = (String) SqlManager.getValueFromVectorParam(datiFS4, 11).getValue();
    if (reference != null) {
      ValidazioneUtility.validazioneS100(reference, "W3FS4S2", "REFERENCE", pagina, listaControlli);
    }

    String type_contract = (String) SqlManager.getValueFromVectorParam(datiFS4, 3).getValue();

    // CPV principale
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.2", null, false);
    String cpv = (String) SqlManager.getValueFromVectorParam(datiFS4, 2).getValue();
    if (cpv == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "CPV", pagina);
    }

    // Controllo corrispondenza CPV con il tipo appalto
    ValidazioneUtility.validazioneCorrispondenzaCPV(listaControlli, pagina, cpv, type_contract, "W3FS4S2", "CPV");

    // Tipo di appalto
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.3", null, false);
    if (type_contract == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "TYPE_CONTRACT", pagina);
    }

    // Breve descrizione
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.4", null, false);
    String scope_total = (String) SqlManager.getValueFromVectorParam(datiFS4, 4).getValue();
    if (scope_total == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "SCOPE_TOTAL", pagina);
    } else {
      ValidazioneUtility.validazioneS1000(scope_total, "W3FS4S2", "SCOPE_TOTAL", pagina, listaControlli);
    }

    // Valore stimato
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.5", null, false);
    Double scope_cost = (Double) SqlManager.getValueFromVectorParam(datiFS4, 10).getValue();
    ValidazioneUtility.validazioneCost(scope_cost, "W3FS4S2", "SCOPE_COST", pagina, listaControlli);

    // Appalto suddiviso in lotti ?
    pagina = UtilityTags.getResource("label.simap.fs4.II.1.6", null, false);
    String div_into_lots = (String) SqlManager.getValueFromVectorParam(datiFS4, 5).getValue();
    if (div_into_lots == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "DIV_INTO_LOTS", pagina);
    } else if ("1".equals(div_into_lots)) {
      String div_lots = (String) SqlManager.getValueFromVectorParam(datiFS4, 6).getValue();
      Long div_lots_max = (Long) SqlManager.getValueFromVectorParam(datiFS4, 7).getValue();
      Long lots_max_tenderer = (Long) SqlManager.getValueFromVectorParam(datiFS4, 8).getValue();

      if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
        if (div_lots != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4S2", "DIV_LOTS", pagina);
        if (div_lots_max != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4S2", "DIV_LOTS_MAX", pagina);
        if (lots_max_tenderer != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4S2", "LOTS_MAX_TENDERER", pagina);

      } else {
        // Le offerte vanno presentare per tutti i lotti, solo ...
        if (div_lots == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "DIV_LOTS", pagina);
        }

        // Numero massimo di lotti su cui presentare le offerte
        if (div_lots != null && "2".equals(div_lots)) {
          if (div_lots_max == null) {
            ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "DIV_LOTS_MAX", pagina);
          } else {
            ...
            if (cntW3ANNEXB != null && div_lots_max.longValue() > cntW3ANNEXB.longValue()) {
              String descrizione = ValidazioneUtility.getDescrizioneCampo("W3FS4S2", "DIV_LOTS_MAX");
        ...
        if (lots_max_tenderer == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "LOTS_MAX_TENDERER", pagina);
        } else {
          Long cntW3ANNEXB = (Long) sqlManager.getObject("select count(*) from w3annexb where id = ?", new Object[] { id });
          if (cntW3ANNEXB != null && lots_max_tenderer.longValue() > cntW3ANNEXB.longValue()) {
            String descrizione = ValidazioneUtility.getDescrizioneCampo("W3FS4S2", "LOTS_MAX_TENDERER");
            String messaggio = "Il numero di lotti indicato deve essere inferiore o uguale al numero totale dei lotti";
            listaControlli.add(((Object) (new Object[] { "E", pagina, descrizione, messaggio })));
          }
      ...
      String lots_combining = (String) SqlManager.getValueFromVectorParam(datiFS4, 12).getValue();
      if (lots_combining != null) {
        ValidazioneUtility.validazioneS400(lots_combining, "W3FS4S2", "LOTS_COMBINING", pagina, listaControlli);
      }

    }

    // Data prevista pubblicazione
    pagina = UtilityTags.getResource("label.simap.fs4.II.3", null, false);
    Date date_publi = (Date) SqlManager.getValueFromVectorParam(datiFS4, 9).getValue();
    if (NOTICE_F04_PRI_CALL_COMPETITION.equals(notice_f04)) {
      if (date_publi != null) {
        ValidazioneUtility.addNotForeseenF01PriCallCompetition(listaControlli, "W3FS4S2", "DATE_PUBLI", pagina);
      }
    } else {
      if (date_publi == null) {
        ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4S2", "DATE_PUBLI", pagina);
      }
    }
   ...
   * @throws GestoreException
   */
  public void validazione_FS4_W3ANNEXB(SqlManager sqlManager, Long id, Long w3annexb_num, List<Object> listaControlli)
      throws GestoreException {
    ...
    try {
      ...
      if (datiW3ANNEXB != null && datiW3ANNEXB.size() > 0) {
        ...
        for (int i = 0; i < datiW3ANNEXB.size(); i++) {
          ...
          if (NOTICE_F01_PRI_ONLY.equals(notice_f04)) {
            if (options != null) {
   ...
   * @throws GestoreException
   */
  private void validazioneFS4_Sezione_III(SqlManager sqlManager, Long id, List<Object> listaControlli) throws SQLException,
      GestoreException {

    String selectFS4 = "select eco_criteria_doc, " // 0
        + " eco_fin_info, " // 1
        + " eco_fin_min_level, " // 2
        + " tech_crit_doc, " // 3
        + " tech_prof_info, " // 4
    ...
    String suitability = (String) SqlManager.getValueFromVectorParam(datiFS4, 6).getValue();
    pagina = UtilityTags.getResource("label.simap.fs4.III.1.1", null, false);
    ValidazioneUtility.validazioneS4000(suitability, "W3FS4", "SUITABILITY", pagina, listaControlli);

    String eco_fin_info = (String) SqlManager.getValueFromVectorParam(datiFS4, 1).getValue();
    String eco_fin_min_level = (String) SqlManager.getValueFromVectorParam(datiFS4, 2).getValue();
    pagina = UtilityTags.getResource("label.simap.fs4.III.1.2", null, false);
    ValidazioneUtility.validazioneS4000(eco_fin_info, "W3FS4", "ECO_FIN_INFO", pagina, listaControlli);
    ValidazioneUtility.validazioneS4000(eco_fin_min_level, "W3FS4", "ECO_FIN_MIN_LEVEL", pagina, listaControlli);

    String tech_prof_info = (String) SqlManager.getValueFromVectorParam(datiFS4, 4).getValue();
    String tech_prof_min_level = (String) SqlManager.getValueFromVectorParam(datiFS4, 5).getValue();
    pagina = UtilityTags.getResource("label.simap.fs4.III.1.3", null, false);
    ValidazioneUtility.validazioneS4000(tech_prof_info, "W3FS4", "TECH_PROF_INFO", pagina, listaControlli);
    ValidazioneUtility.validazioneS4000(tech_prof_min_level, "W3FS4", "TECH_PROF_MIN_LEVEL", pagina, listaControlli);

    pagina = UtilityTags.getResource("label.simap.fs4.III.2.1", null, false);
    String reference_to_law = (String) SqlManager.getValueFromVectorParam(datiFS4, 10).getValue();
    ValidazioneUtility.validazioneS1500(reference_to_law, "W3FS4", "REFERENCE_TO_LAW", pagina, listaControlli);

    pagina = UtilityTags.getResource("label.simap.fs4.III.2.2", null, false);
    String performance_conditions = (String) SqlManager.getValueFromVectorParam(datiFS4, 11).getValue();
    ValidazioneUtility.validazioneS1000(performance_conditions, "W3FS4", "PERFORMANCE_CONDITIONS", pagina, listaControlli);

    Long notice_f04 = (Long) sqlManager.getObject("select notice_f04 from w3fs4 where id = ?", new Object[] { id });
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {

      pagina = UtilityTags.getResource("label.simap.fs4.III.1.1", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 6).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "SUITABILITY", pagina);
      }

      pagina = UtilityTags.getResource("label.simap.fs4.III.1.2", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 0).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "ECO_CRITERIA_DOC", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 1).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "ECO_FIN_INFO", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 2).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "ECO_FIN_MIN_LEVEL", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 3).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "TECH_CRIT_DOC", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 4).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "TECH_PROF_INFO", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 5).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "TECH_PROF_MIN_LEVEL", pagina);
      }

      pagina = UtilityTags.getResource("label.simap.fs4.III.1.5", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 7).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "RESTRICTED_SHELTERED", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 8).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "RESTRICTED_FRAME", pagina);
      }

      pagina = UtilityTags.getResource("label.simap.fs4.III.2.1", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 9).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "PARTICULAR_PROFESSION", pagina);
      }
      if (SqlManager.getValueFromVectorParam(datiFS4, 10).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "REFERENCE_TO_LAW", pagina);
      }

      pagina = UtilityTags.getResource("label.simap.fs4.III.2.2", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 11).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "PERFORMANCE_CONDITIONS", pagina);
      }

      pagina = UtilityTags.getResource("label.simap.fs4.III.2.3", null, false);
      if (SqlManager.getValueFromVectorParam(datiFS4, 12).getValue() != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "PERFORMANCE_STAFF", pagina);
      }
      ...
      pagina = UtilityTags.getResource("label.simap.fs4.III.1.2", null, false);
      String eco_criteria_doc = (String) SqlManager.getValueFromVectorParam(datiFS4, 0).getValue();
      if (eco_criteria_doc != null && "2".equals(eco_criteria_doc)) {
        if (SqlManager.getValueFromVectorParam(datiFS4, 1).getValue() == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "ECO_FIN_INFO", pagina);
        }
        if (SqlManager.getValueFromVectorParam(datiFS4, 2).getValue() == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "ECO_FIN_MIN_LEVEL", pagina);
        }
      }

      // Capacita' professionale e tecnica
      pagina = UtilityTags.getResource("label.simap.fs4.III.1.3", null, false);
      String tech_crit_doc = (String) SqlManager.getValueFromVectorParam(datiFS4, 3).getValue();
      if (tech_crit_doc != null && "2".equals(tech_crit_doc)) {
        if (SqlManager.getValueFromVectorParam(datiFS4, 4).getValue() == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "TECH_PROF_INFO", pagina);
        }
        if (SqlManager.getValueFromVectorParam(datiFS4, 5).getValue() == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "TECH_PROF_MIN_LEVEL", pagina);
        }
      }
    }
  }
   ...
   * @throws GestoreException
   */
  private void validazioneFS4_Sezione_IV(SqlManager sqlManager, Long id, List<Object> listaControlli) throws SQLException, GestoreException {

    String selectFS4 = "select type_procedure, " // 0
        + " framework, " // 1
        + " fr_several_op, " // 2
        + " gpa, " // 3
        + " date_receipt, " // 4
    ...
    // Tipo di procedura
    Long type_procedure = (Long) SqlManager.getValueFromVectorParam(datiFS4, 0).getValue();
    pagina = UtilityTags.getResource("label.simap.fs4.IV.1.1", null, false);

    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (type_procedure != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "TYPE_PROCEDURE", pagina);
    } else if (NOTICE_F04_PRI_REDUCING_TIME_LIMITS.equals(notice_f04)) {
      if (type_procedure != null)
        ValidazioneUtility.addNotForeseenF01PriReducingTimeLimits(listaControlli, "W3FS4", "TYPE_PROCEDURE", pagina);
    } else {
      if (type_procedure == null) {
        ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "TYPE_PROCEDURE", pagina);
      }
    }

    // Numero di operatori nel caso di accordo quadro
    pagina = UtilityTags.getResource("label.simap.fs4.IV.1.3", null, false);
    String framework = (String) SqlManager.getValueFromVectorParam(datiFS4, 1).getValue();
    String fr_several_op = (String) SqlManager.getValueFromVectorParam(datiFS4, 2).getValue();
    String fr_justification = (String) SqlManager.getValueFromVectorParam(datiFS4, 2).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (framework != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "FRAMEWORK", pagina);
    } else {
      if (framework != null && "1".equals(framework)) {
        if (fr_several_op == null) {
          ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "FR_SEVERAL_OP", pagina);
        }
        ValidazioneUtility.validazioneS400(fr_justification, "W3FS4", "FR_JUSTIFICATION", pagina, listaControlli);
      }
    }

    // Sistema dinamico di acquisizione
    String dps = (String) SqlManager.getValueFromVectorParam(datiFS4, 7).getValue();
    if (dps != null && NOTICE_F01_PRI_ONLY.equals(notice_f04)) {
      ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "DPS", pagina);
    }

    // Asta elettronica ?
    pagina = UtilityTags.getResource("label.simap.fs4.IV.1.6", null, false);
    String eauction = (String) SqlManager.getValueFromVectorParam(datiFS4, 8).getValue();
    if (eauction != null && NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "EAUCTION", pagina);
    }
    String eauction_info = (String) SqlManager.getValueFromVectorParam(datiFS4, 12).getValue();
    ValidazioneUtility.validazioneS400(eauction_info, "W3FS4", "EAUCTION", pagina, listaControlli);

    // Accordo sugli appalti pubblici (AAP)
    pagina = UtilityTags.getResource("label.simap.fs4.IV.1.8", null, false);
    if (SqlManager.getValueFromVectorParam(datiFS4, 3).getValue() == null) {
      ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "GPA", pagina);
    }

    // Termine ricezione manifestazione di interesse
    pagina = UtilityTags.getResource("label.simap.fs4.IV.2.2", null, false);
    Date date_receipt = (Date) SqlManager.getValueFromVectorParam(datiFS4, 4).getValue();
    String time_receipt = (String) SqlManager.getValueFromVectorParam(datiFS4, 9).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (date_receipt != null) ValidazioneUtility.addNotForeseenF04PriOnly(listaControlli, "W3FS4", "DATE_RECEIPT", pagina);
      if (time_receipt != null) ValidazioneUtility.addNotForeseenF04PriOnly(listaControlli, "W3FS4", "TIME_RECEIPT", pagina);
    } else if (NOTICE_F04_PRI_REDUCING_TIME_LIMITS.equals(notice_f04)) {
      if (date_receipt == null) ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "DATE_RECEIPT", pagina);
      if (time_receipt == null) ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "TIME_RECEIPT", pagina);
    } else {
      if (date_receipt == null) ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "DATE_RECEIPT", pagina);
      if (time_receipt == null) ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "TIME_RECEIPT", pagina);
    }
    ...
    if (date_receipt != null) {
      // Date notice_date = (Date) SqlManager.getValueFromVectorParam(datiFS4,
      // 11).getValue();
      Date notice_date = UtilityDate.getDataOdiernaAsDate();
      if (notice_date != null) {
        if (date_receipt.getTime() <= notice_date.getTime()) {
          String messaggio = ValidazioneUtility.getMessaggioConfrontoDate(date_receipt, ">", notice_date, "W3FS4", "NOTICE_DATE");
          ValidazioneUtility.addAvviso(listaControlli, "W3FS4", "DATE_RECEIPT", "E", pagina, messaggio);
        }
      } else {
        Date notice_date_today = new Date();
        if (date_receipt.getTime() <= notice_date_today.getTime()) {
          String messaggio = ValidazioneUtility.getMessaggioConfrontoDate(date_receipt, ">", notice_date_today, "W3FS4", "NOTICE_DATE");
          ValidazioneUtility.addAvviso(listaControlli, "W3FS4", "DATE_RECEIPT", "E", pagina, messaggio);
        }
      }
    }
    ...
    // Lingue
    pagina = UtilityTags.getResource("label.simap.fs4.IV.2.4", null, false);
    ...
    Date date_award = (Date) SqlManager.getValueFromVectorParam(datiFS4, 10).getValue();
    if (date_award != null && NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      ValidazioneUtility.addNotForeseenF04PriOnly(listaControlli, "W3FS4", "DATE_AWARD", pagina);
    }
   ...
   * @throws GestoreException
   */
  private void validazioneFS4_Sezione_VI(SqlManager sqlManager, Long id, List<Object> listaControlli) throws SQLException, GestoreException {
    String selectFS4 = "select review_body_codein, " // 0
    ...
    pagina = UtilityTags.getResource("label.simap.fs4.VI.2", null, false);
    String eordering = (String) SqlManager.getValueFromVectorParam(datiFS4, 4).getValue();
    String einvoicing = (String) SqlManager.getValueFromVectorParam(datiFS4, 5).getValue();
    String epayment = (String) SqlManager.getValueFromVectorParam(datiFS4, 6).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (eordering != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "EORDERING", pagina);
      if (einvoicing != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "EINVOICING", pagina);
      if (epayment != null) ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "EPAYMENT", pagina);
    }

    // Informazioni complementari
    String additional_information = (String) SqlManager.getValueFromVectorParam(datiFS4, 8).getValue();
    ValidazioneUtility.validazioneS4000(additional_information, "W3FS4", "ADDITIONAL_INFORMATION", pagina, listaControlli);

    // Organismo responsabile delle procedure di ricorso
    pagina = UtilityTags.getResource("label.simap.fs4.VI.4.1", null, false);
    String review_body_codein = (String) SqlManager.getValueFromVectorParam(datiFS4, 0).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (review_body_codein != null) {
        ValidazioneUtility.addNotForeseenF04PriOnly(listaControlli, "W3FS4", "REVIEW_BODY_CODEIN", pagina);
      }
    } else {
      if (review_body_codein == null) {
        ValidazioneUtility.addCampoObbligatorio(listaControlli, "W3FS4", "REVIEW_BODY_CODEIN", pagina);
      } else {
    ...
    pagina = UtilityTags.getResource("label.simap.fs4.VI.4.2", null, false);
    String mediation_body_codein = (String) SqlManager.getValueFromVectorParam(datiFS4, 1).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (mediation_body_codein != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "MEDIATION_BODY_CODEIN", pagina);
      }
    } else {
      if (mediation_body_codein != null) {
    ...
    pagina = UtilityTags.getResource("label.simap.fs4.VI.4.3", null, false);
    String review_procedure = (String) SqlManager.getValueFromVectorParam(datiFS4, 7).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (review_procedure != null) {
        ValidazioneUtility.addNotForeseenF01PriOnly(listaControlli, "W3FS4", "REVIEW_PROCEDURE", pagina);
      }
    }
    ValidazioneUtility.validazioneS4000(review_procedure, "W3FS4", "REVIEW_PROCEDURE", pagina, listaControlli);

    // Servizio presso il quale sono disponibili...
    pagina = UtilityTags.getResource("label.simap.fs4.VI.4.4", null, false);
    String review_info_codein = (String) SqlManager.getValueFromVectorParam(datiFS4, 2).getValue();
    if (NOTICE_F04_PRI_ONLY.equals(notice_f04)) {
      if (review_info_codein != null) {
        ValidazioneUtility.addNotForeseenF04PriOnly(listaControlli, "W3FS4", "REVIEW_INFO_CODEIN", pagina);
      }
    } else {
      if (review_info_codein != null) {
